...
class BI_DLL $modify(BILevelCell, LevelCell) {
    ...
    void loadCustomLevelCell() {
         ...
         * BI+ Visibility badge
        */
        if((m_level->m_unlisted || m_level->m_friendsOnly) && Mod::get()->getSettingValue<bool>("bip-visibility-badges")) {
            auto badgeNode = CCLabelBMFont::create(m_level->m_friendsOnly ? "FRIENDS ONLY" : "UNLISTED", "chatFont.fnt");

            float badgeX = idTextNode->getPositionX();
            if(Mod::get()->getSettingValue<bool>("show-level-ids")) badgeX -= idTextNode->getScaledContentSize().width + 3.f;

            badgeNode->setPosition({badgeX, idTextNode->getPositionY()});
            badgeNode->setAnchorPoint({1,1});
            badgeNode->setScale(idTextNode->getScale());
            badgeNode->setColor(m_level->m_friendsOnly ? ccColor3B{141,255,141} : ccColor3B{255,172,76});
            badgeNode->setOpacity(200);
            badgeNode->setID("visibility-badge"_spr);
